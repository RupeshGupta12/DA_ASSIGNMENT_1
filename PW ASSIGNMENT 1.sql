...
--- Primary Key: A primary key is a column or a set of columns that uniquely identifies each row in a table. It must contain unique values and cannot contain NULL values. It is used to uniquely identify each record in a table and to enforce entity integrity. 
--- Foreign Key: A foreign key is a column or a set of columns in a table that refers to the primary key of another table. It can contain NULL values, which indicates that the relationship is optional. It establishes a link between the two tables and enforces referential integrity. 
